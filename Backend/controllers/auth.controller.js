...
exports.signup = async (req, res) => {
  ...
  const emailSent = await sendEmailVerification({ to: user.email, name, code: emailCode });
  if (!emailSent) {
    await Promise.all([
      User.deleteOne({ _id: user._id }),
      UserRole.deleteOne({ user_id: user._id }),
...
exports.requestEmailVerification = async (req, res) => {
  ...
  const emailSent = await sendEmailVerification({ to: user.email, code: emailCode });
  if (!emailSent) {
    return res.status(500).json({ message: 'Unable to send verification email. Please try again later.' });
  }
